<?php 
	 ...
	 if(isset($_POST['class_test'])	  ||  isset($_GET['class_test_results']) 	)  
	{
...
$test="no";
			...
			}else if(isset($_GET['class_test_results'])){
	...
	if($top_ridge->get_class_total_count($class_id) > 0)
		{
		  echo "<input type='submit' title='Click to save all records'  class='submit_button save_button ui-corner-all' value='Save Record' name='class_test_results' /> ";
	...
	<th axis='string' width='10%' class='center'>CAT 1 <br />(15)</th>
	<th axis='string' width='10%' class='center'>CAT 2 <br />(15)</th>
<!--	<th axis='string' width='10%' class='center'>CT 3 <br />(20)</th>-->
	
	<th axis='string' width='10%' class='center'>SUB TOTAL <br />(30)</th>
	...
	CONCAT(students.last_name,' ',first_name) AS STUDENT_NAME,class_test_one,
	class_test_two, (class_test_one + class_test_two) AS TOTAL 
	...
	while($rows = mysqli_fetch_assoc($home_work_results))
	{
	   ...
	   $student_id = $rows['id'];
		...
		$class_test_one =$rows['class_test_one'];
		$class_test_two = $rows['class_test_two'];


//		$class_test_three = $rows['class_test_three'];
		...
		if($class_test_one < 0 || $class_test_one >15){
			echo " style='background-color: red' ";}
		echo "type='text'  name='class_test_one[]' value=$class_test_one></td>
		
		
		
		
		<td class='center'><input ";
		if($class_test_two < 0 || $class_test_two >15){
			echo " style='background-color: red' ";}
		echo  " type='text' name='class_test_two[]' value=$class_test_two ></td>"
.
//		<td class='center'><input type='text' name='class_test_three[]' value=$class_test_three /></td>
...
//		<td class='center'><input type='text' name='class_test_three[]' value=$class_test_three /></td>
		...
		echo "<td class='center' style='vertical-align:middle'><b>$TOTAL</b></td>
